...
from file_manager.core.config_manager import ROOT_PATH
from file_manager.core.api import make_db, write_config, publish_folder
...
from file_manager.core.config_manager.config_rw import parse_config, get_attributes_only, write_config_to_file
...
class CopyFrame(tk.Frame):
    ...
    def ask_dir_from(self):
        ...
        if path:
            if path_is_parent(ROOT_PATH, path):
                msgbox.showerror(
                    'ERROR', f'"From:" folder must be outside root directory: {ROOT_PATH}')
                return
    ...
    def ask_dir_to(self):
        initial_dir = ROOT_PATH if self.path_to == '' else self.path_to
        path = filedialog.askdirectory(initialdir=initial_dir)
        if path:
            if not path_is_parent(ROOT_PATH, path):
                msgbox.showerror(
                    'ERROR', f'"To:" folder must be inside root directory: {ROOT_PATH}')
                return
            self.path_to = path
            self.rel_path_to = os.path.relpath(self.path_to, ROOT_PATH)
